            ...
            if len(lektor) > 0:
                ...
                for link in links:
                    if str(link).startswith("http://"):
                        ...
                        sources.append({'source': host, 'quality': q, 'language': 'pl', 'url': link, 'info': 'Lektor',
            ...
            if len(napisy) > 0:
                ...
                for link in links:
                    if str(link).startswith("http://"):
                        ...
                        valid, host = source_utils.is_host_valid(link, hostDict)
                        if not valid: continue
                        sources.append({'source': host, 'quality': q, 'language': 'pl', 'url': link, 'info': 'Napisy',
                                        'direct': False, 'debridonly': False})
            ...
            if len(eng) > 0:
                ...
                for link in links:
                    if str(link).startswith("http://"):
                        ...
                        valid, host = source_utils.is_host_valid(link, hostDict)
                        if not valid: continue
                        sources.append({'source': host, 'quality': q, 'language': 'en', 'url': link, 'info': 'ENG',
                                        'direct': False, 'debridonly': False})
            debug = 1;
            return sources
        except Exception as e:

...
class source:
    ...
    def resolve(self, url):
        try:
            ...
            p1 = self.scraper.post('http://gostream.sc/ip.file/swf/plugins/ipplugins.php', data=post,
                                  headers={'referer':urldata['url']}).content
            p1 = json.loads(p1)
            p2 = self.scraper.get('http://gostream.sc/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=0' % (
            ...
            n = p3['status']
            if n == False:
                p2 = self.scraper.get('http://gostream.sc/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=1' % (
                    p1['s'], urldata['data-server'])).content
                p2 = json.loads(p2)

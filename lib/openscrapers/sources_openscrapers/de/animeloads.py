			...
			for id, info in rels:
				...
				for html, quality in rel:
					try:
						...
						enc = \
						dom_parser.parse_dom(html, 'div', attrs={'id': re.compile('streams_episodes_%s_%s' % (id, s))},
						...
						hosters = \
						[re.findall('hoster-(\w+)', ' '.join([x.attrs['class'] for x in i])) for i in hosters if i][0]

...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            post = 'do=search&subaction=search&search_start=0&full_search=0&result_from=1&story=' + urllib.quote_plus(
                title) + ' ' + urllib.quote_plus(imdb)
            # Join site links
            ...
            for link in actual_links:
                quality, info = source_utils.get_release_quality(link,
                                                                 url)  # Run two strings through this to try to pull Quality and info on the file (such as 1080p, etc)
                host = link.replace('https://', '').split('/')[0].split('.')[
                    0]  # By splitting the URL repeatedly we end up with the host.
                sources.append(
